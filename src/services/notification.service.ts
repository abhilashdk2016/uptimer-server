...
export async function getSingleNotificationGroup(notificationId: number): Promise<INotificationDocument | undefined> {
    try {
        const notifications: INotificationDocument | undefined = NotificationModel.findOne({
            raw: true,
            where: {
                id: notificationId
            },
        ...
        }) as unknown as INotificationDocument | undefined;
...
export async function getAllNotificationGroups(userId: number): Promise<INotificationDocument[]> {
    try {
        const notifications: INotificationDocument[] = NotificationModel.findAll({
            raw: true,
            where: {
                userId
            },
...
export async function updateNotificationGroup(notificationId: number, data: INotificationDocument): Promise<void> {
    try {
        await NotificationModel.update(data, { where: { id: notificationId }});
    } catch (error) {
        throw new Error(error);
    }
}
